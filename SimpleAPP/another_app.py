...
class ChatBot:
    ...
    def get_gemini_response_special(self,user_input,db_result, conversation_history, previous_db_results):
        question =""" **Task:**
        Generate a clear and concise response, tailored to the user's query. 
        - If the AI response is a single value, provide a direct answer.
        - If the AI response is a list or table, present the information in a well-formatted table with appropriate headers."""
        formatted_results = self.format_results(previous_db_results)
    ...
    def send_prompts(self, user_input, previous_db_results, temperature=0.5):
        ...
        if user_input.lower() not in bypass_db_queries:
            try:
                ...
                response = self.get_gemini_response(question = user_input, conversation_history=self._conversation_history, previous_db_results=previous_db_results)
